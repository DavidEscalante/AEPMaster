...
set P3renderPSD to (AErenderengine & space & "-project" & space & inputFilePath & space & "-comp" & space & quoted form of AEPname & space & "-RStemplate 'Multi-Machine Settings' -OMtemplate 'Multi-Machine Sequence' -output" & space & quoted form of (RENDERfolder & "/" & "_OUTPUT" & "/[#####].psd") & "; exit")

-- FMPEG
set P4renderMOV to (quoted form of (myAPPpath & "/Contents/Resources/ffmpeg") & space & "-f image2 -r" & space & AEPfps & space & "-i" & space & "'" & RENDERfolder & "/" & "_OUTPUT" & "/%05d.psd" & "'" & space & "-i" & space & "'" & RENDERfolder & "/" & "_OUTPUT" & "/audio.aif" & "'" & space & "-shortest -c:v prores_ks -profile:v 3 -r" & space & AEPfps & space & "'" & RENDERfolder & "/" & AEPname & ".mov" & "'" & space & "-y")
-- MP4:PRE
set P5MP4 to (quoted form of (myAPPpath & "/Contents/Resources/ffmpeg") & space & "-f image2 -r" & space & AEPfps & space & "-i" & space & "'" & RENDERfolder & "/" & "_OUTPUT" & "/%05d.psd" & "'" & space & "-i" & space & "'" & RENDERfolder & "/" & "_OUTPUT" & "/audio.aif" & "'" & space & "-pix_fmt yuv420p -c:v libx264 -preset veryslow -crf 12 -r" & space & AEPfps & space & "'" & RENDERfolder & "/" & AEPname & ".mp4" & "'" & space & "-y")
-- MP4:POST
set P5MP4consonido to (quoted form of (myAPPpath & "/Contents/Resources/ffmpeg") & space & "-i" & space & "'" & RENDERfolder & "/" & AEPname & ".mp4" & "'" & space & "-i" & space & "'" & RENDERfolder & "/" & "_OUTPUT" & "/audio.aif" & "'" & space & "-c:v copy -map 0:v:0 -map 1:a:0" & space & "'" & SAVEFINAL & "/" & AEPname & ".mp4" & "'" & space & "-y")
...
if NUMBERofinstances = "2" then
	tell application "iTerm"
		...
		tell current session of S1
			...
			set S2 to split vertically with profile "Default"
...
if NUMBERofinstances = "3" then
	tell application "iTerm"
		...
		tell current session of S1
			...
			set S3 to split vertically with profile "Default"
...
if NUMBERofinstances = "4" then
	tell application "iTerm"
		...
		tell current session of S1
			...
			set S4 to split vertically with profile "Default"
...
if NUMBERofinstances = "5" then
	tell application "iTerm"
		...
		tell current session of S1
			...
			set S5 to split vertically with profile "Default"
...
if NUMBERofinstances = "6" then
	tell application "iTerm"
		...
		tell current session of S1
			...
			set S6 to split vertically with profile "Default"
...
if NUMBERofinstances = "7" then
	tell application "iTerm"
		...
		tell current session of S1
			...
			set S7 to split vertically with profile "Default"
...
if NUMBERofinstances = "8" then
	tell application "iTerm"
		...
		tell current session of S1
			...
			set S8 to split vertically with profile "Default"
...
if NUMBERofinstances = "9" then
	tell application "iTerm"
		...
		tell current session of S1
			...
			set S9 to split vertically with profile "Default"
...
if NUMBERofinstances = "10" then
	tell application "iTerm"
		...
		tell current session of S1
			...
			set S10 to split vertically with profile "Default"
...
on error
	try
		tell application "iTerm"
			...
			set S1 to create window with profile "ffmpeg"
			tell current session of S1
				set name to (AEPname & space & "MP4 without sound")
				write text P5MP4
	...
	on error
		beep 1
		display notification (AEPname & ": ERROR (MP4 without sound)")
		error number -128 -- user canceled
	end try
end try
...
on error
	...
	on error
		beep 1
		display notification (AEPname & ": ERROR (MP4 sound merge)")
		error number -128 -- user canceled
	end try
end try
